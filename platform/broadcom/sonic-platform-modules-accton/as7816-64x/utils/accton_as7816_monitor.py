...
class accton_as7816_monitor(object):
    ...
    def __init__(self, log_console, log_file, log_level=logging.INFO):
        ...
        self.llog.setLevel(logging.WARNING)       
    ...
    def manage_fans(self):
        ...
        for x in range(fan.get_idx_fan_start(), fan.get_num_fans()+1):
            ...
            if fan_status is None:
                self.llog.error('SET new_perc to %d (FAN stauts is None. fan_num:%d)', max_duty, x)
        ...
        for x in range(0, len(fan_policy)):
            ...
            if get_temp > fan_policy[y][1] and get_temp < fan_policy[y][2] :
                ...
                if (new_duty_cycle == max_duty):
                    self.llog.warning('Full speed on for high temp: %d',get_temp)

                self.llog.info('INFO. Sum of temp %d > %d , new_duty_cycle=%d', get_temp, fan_policy[y][1], new_duty_cycle)

        self.llog.info('INFO. Final duty_cycle=%d', new_duty_cycle)
