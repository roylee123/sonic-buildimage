...
class accton_as7816_monitor(object):
    ...
    def manage_thermal(self):
        thermal = ThermalUtil()
        temp_4a = thermal._get_thermal_node_val(3)            
        if temp_4a >= 70000:
            self.shutdown()
    ...
    def manage_fans(self):
        ...
        for x in range(fan.get_idx_fan_start(), fan.get_num_fans()+1):
            ...
            if fan_status is None:
                self.llog.error('INFO. SET new_perc to %d (FAN stauts is None. fan_num:%d)', max_duty, x)
                return False
            if fan_status is False:             
                self.llog.warning('INFO. SET new_perc to %d (FAN fault. fan_num:%d)', max_duty, x)
